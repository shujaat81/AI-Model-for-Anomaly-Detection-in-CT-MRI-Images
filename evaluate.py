...
from models.autoencoder import (
    ...
    detect_anomalies,
    compute_reconstruction_error_in_batches,
)
...
def evaluate_autoencoder(autoencoder, normal_data, anomaly_data=None):
    ...
    if anomaly_data is not None:
        ...
        y_true = np.concatenate(
            [np.zeros(len(normal_recon_errors)), np.ones(len(anomaly_recon_errors))]
        )
        ...
        roc_auc = plot_roc_curve(
            y_true, y_scores, save_path=RESULTS_DIR / "autoencoder_roc_curve.png"
        )

        print(f"Autoencoder ROC AUC: {roc_auc:.4f}")
        print(f"Autoencoder PR AUC: {metrics['pr_auc']:.4f}")
        print(
            f"Best F1 Score: {metrics['best_f1']:.4f} at threshold {metrics['best_threshold']:.6f}"
        )
...
def extract_features_for_evaluation(feature_extractor, images, batch_size=16):
    ...
    all_features = []
    print(
        f"Extracting features from {total_samples} images in {num_batches} batches..."
    )
...
def compare_methods(autoencoder_metrics, ocsvm_metrics):
    ...
    print("-" * 50)
    print(
        f"{'Autoencoder':<15} {autoencoder_metrics['roc_auc']:<10.4f} {autoencoder_metrics['pr_auc']:<10.4f} {autoencoder_metrics['best_f1']:<10.4f}"
    )
    print(
        f"{'One-Class SVM':<15} {ocsvm_metrics['roc_auc']:<10.4f} {ocsvm_metrics['pr_auc']:<10.4f} {ocsvm_metrics['best_f1']:<10.4f}"
    )
    print("-" * 50)
...
def main():
    ...
    print(
        f"Loaded {len(normal_data)} normal images and {len(anomaly_data)} anomaly images for evaluation."
    )

    # Evaluate autoencoder
    if autoencoder is not None:
        autoencoder_metrics = evaluate_autoencoder(
            autoencoder, normal_data, anomaly_data
        )
    else:
    ...
    if ocsvm is not None:
        ...
        ocsvm_metrics = evaluate_one_class_svm(
            ocsvm, feature_extractor, normal_data, anomaly_data
        )
    else:
        ocsvm_metrics = None
        ...
        if os.path.exists(demo_dir):
            ...
            for img_path in os.listdir(demo_dir)[:10]:  # Limit to 10 images
                if img_path.endswith((".jpg", ".png", ".jpeg")):
                    img = tf.keras.preprocessing.image.load_img(
                        os.path.join(demo_dir, img_path), target_size=IMAGE_SIZE
                    )
            ...
            if demo_images:
                ...
                if autoencoder is not None and autoencoder_metrics is not None:
                    ...
                    unseen_scores = compute_reconstruction_error_in_batches(
                        autoencoder, demo_images
                    )
                    ...
                    print(
                        f"Evaluated {len(demo_images)} demo images. Results saved to {RESULTS_DIR / 'demo_predictions.png'}"
                    )
            else:
